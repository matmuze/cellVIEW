...
{		
	...
	// Find lod level	
	int endLoop = (enableLod == 0) ? 0 : numLevels -1;				
	for(int i = 0; i < endLoop; i++) { lodLevel = cameraDistance < _LodLevelsInfos[i][0] ? lodLevel : i+1; }	
	...
	output.radiusScale = (lodLevel == 0 ? 1 : 1.45) * _Scale;
	output.decimationFactor = (lodLevel == 0 ? 1 : 3);
